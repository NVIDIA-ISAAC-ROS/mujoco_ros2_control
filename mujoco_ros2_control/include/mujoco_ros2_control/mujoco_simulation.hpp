...
public:
  ...
  using PluginVisualizationCallback = std::function<void(const mjModel*, const mjData*, mjvScene*)>;
  using PluginKeyCallback = std::function<bool(int key, int scancode, int action, int mods)>;
  ...
  /**
   * @brief Register hooks for plugin viewer geometry and keyboard handling.
   */
  void set_plugin_callbacks(PluginVisualizationCallback visualization_callback, PluginKeyCallback key_callback);
  ...
  /**
   * @brief Enable or disable controller-driven simulation stepping.
   *
   * Enabling lockstep pauses free-running simulation. Callers then advance
   * physics explicitly with request_simulation_steps().
   */
  void configure_lockstep(bool enabled);

  /**
   * @brief Request an exact number of simulation steps and wait for completion.
   */
...
private:
  ...
  PluginVisualizationCallback plugin_visualization_callback_;
  PluginKeyCallback plugin_key_callback_;
