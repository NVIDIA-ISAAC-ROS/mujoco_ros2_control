...
{
public:
  ROS2ControlGlfwAdapter(std::atomic<bool>& step_requested, MujocoSimulation::PluginKeyCallback& plugin_key_callback,
                         std::mutex& plugin_callbacks_mutex)
    : step_requested_(step_requested)
    , plugin_key_callback_(plugin_key_callback)
    , plugin_callbacks_mutex_(plugin_callbacks_mutex)
  {
  }
...
protected:
  void OnKey(int key, int scancode, int act) override
  {
    ...
    bool consumed = false;
    {
      std::lock_guard<std::mutex> lock(plugin_callbacks_mutex_);
      if (plugin_key_callback_)
      {
        consumed = plugin_key_callback_(key, scancode, act, 0);
      }
    }
    if (!consumed)
    {
...
private:
  ...
  MujocoSimulation::PluginKeyCallback& plugin_key_callback_;
  std::mutex& plugin_callbacks_mutex_;
};
                                  ...
                                  const std::string& mujoco_model_topic, double sim_speed_factor, bool headless)
{
  ...
  else
  {
    ...
    ui_thread_ = std::thread([this, sim_ready]() {
      sim_ = std::make_unique<mj::Simulate>(std::make_unique<ROS2ControlGlfwAdapter>(keyboard_step_requested_,
                                                                                     plugin_key_callback_,
                                                                                     plugin_callbacks_mutex_),
                                            &cam_, &opt_, &pert_,
                                            /* is_passive = */ false);
...
void MujocoSimulation::set_plugin_callbacks(PluginVisualizationCallback visualization_callback,
                                            PluginKeyCallback key_callback)
{
  std::lock_guard<std::mutex> lock(plugin_callbacks_mutex_);
  plugin_visualization_callback_ = std::move(visualization_callback);
  plugin_key_callback_ = std::move(key_callback);
}
                                          ...
                                          std::shared_ptr<mujoco_ros2_control_msgs::srv::SetPause::Response> response)
{
  ...
  {
    response->success = false;
    response->message = "Cannot resume simulation: lockstep mode advances from ros2_control writes.";
                                                                                  ...
                                                                                  std::chrono::milliseconds timeout)
{
  ...
  if (!steps_cv_.wait_for(lock, timeout, [this] { return pending_steps_.load() == 0; }))
  {
...
void MujocoSimulation::physics_loop()
{
  ...
  while (!sim_->exitrequest.load())
  {
...

    {
      ...
      if (mj_model_)
      {
        if (lockstep_.load() && sim_->run)
        {
          RCLCPP_WARN(get_logger(), "Ignoring free-run request while lockstep mode is active.");
          sim_->run = 0;
          sim_->speed_changed = true;
        }
...
void MujocoSimulation::update_sim_display()
{
  ...
  if (plugin_visualization_scene_initialized_)
  {
    ...
    if (plugin_visualization_callback_)
    {
      plugin_visualization_callback_(mj_model_, mj_data_, &plugin_visualization_scene_);
    }
  }
