...
The virtual gantry suspends a robot from a fixed world-frame anchor using a one-sided rope
constraint. The rope catches falls without constraining normal motion while it is slack, and
the robot remains free to swing like a pendulum while it is taut.

When enabled, the anchor is placed above the current attachment point at ``anchor_z`` and the
rope length is initialized to the vertical distance between them. A radial spring force pulls
toward the anchor only when the rope is stretched beyond that length. Damping is applied only
while the rope is extending, avoiding asymmetric energy injection during contraction.

The gantry re-anchors after a simulation reset or when it is re-enabled.
...
.. list-table::
   ...
   * - ``body_name``
     - ``string``
     - ``torso_link``
     - MuJoCo body to which the rope attaches.
   * - ``body_offset``
     - ``double[3]``
     - ``[0, 0, 0]``
     - Attachment offset from the body center of mass, in the body frame [m].
   * - ``kp_pos``
     - ``double``
     - ``5000.0``
     - Rope spring stiffness [N/m].
   * - ``kd_pos``
     - ``double``
     - ``3000.0``
     - Rope damping while the rope is extending [N s/m].
   * - ``anchor_z``
     - ``double``
     - ``1.5``
     - Fixed world-frame anchor height [m].
...
.. list-table::
   ...
   * - Key
     - Action
   * - ``G``
     - Toggle the gantry. Re-enabling re-anchors it above the current attachment point.
   * - ``[``
     - Shorten the rope by 0.5 cm. Ignored while the gantry is disabled.
   * - ``]``
     - Lengthen the rope by 0.5 cm. Ignored while the gantry is disabled.

ROS 2 Interface
^^^^^^^^^^^^^^^

The ``set_gantry_enabled`` service uses ``std_srvs/srv/SetBool``. Its full path is
``/<node_name>/<plugin_key>/set_gantry_enabled``.

.. code-block:: bash

   ros2 service call \
     /mujoco_ros2_control_node/virtual_gantry/set_gantry_enabled \
...
``anchor_z`` controls the rope length at activation. A higher anchor produces a longer rope and
more nearly vertical forces. ``kp_pos`` controls sag under load; for a mass ``m``, static sag is
approximately ``m * g / kp_pos``. ``kd_pos`` controls settling after a fall or rope-length change.
