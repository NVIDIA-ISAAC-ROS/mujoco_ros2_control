...
void CameraPlugin::trigger_update()
{
  {
    std::lock_guard<std::mutex> lock(data_mutex_);
    if (!publish_images_ || !mj_camera_data_)
    {
      return;
    }
    // Force every streaming camera and consume any pending polls. Rendering
    // must stay on rendering_thread_, which owns the OpenGL context.
    for (auto& camera : cameras_)
    {
...
bool CameraPlugin::init_egl_context()
{
  ...
  // Rendering threads can reuse a pthread that previously belonged to a ROS
  // executor. Reset any driver TLS before selecting a display, as MuJoCo's
  // EGL backend does.
  eglReleaseThread();

  // MuJoCo's EGL backend binds directly to an EGL device. In particular, an
  // NVIDIA driver can initialize a surfaceless Mesa display while still being
  // unable to provide the framebuffer that mjr_makeContext requires.
  const auto query_devices = reinterpret_cast<PFNEGLQUERYDEVICESEXTPROC>(eglGetProcAddress("eglQueryDevicesEXT"));
  const auto get_device_display =
      reinterpret_cast<PFNEGLGETPLATFORMDISPLAYEXTPROC>(eglGetProcAddress("eglGetPlatformDisplayEXT"));
  if (query_devices && get_device_display)
  {
    std::array<EGLDeviceEXT, 32> devices{};
    EGLint device_count = 0;
    if (query_devices(static_cast<EGLint>(devices.size()), devices.data(), &device_count))
    {
      int first_device = 0;
      int last_device = device_count;
      if (const char* device_id = std::getenv("MUJOCO_EGL_DEVICE_ID"))
      {
        char* end = nullptr;
        const long requested_device = std::strtol(device_id, &end, 10);
        if (end == device_id || *end != '\0' || requested_device < 0 || requested_device >= device_count)
        {
          RCLCPP_ERROR(node_->get_logger(), "MUJOCO_EGL_DEVICE_ID must be between 0 and %d, got '%s'", device_count - 1,
                       device_id);
          return false;
        }
        first_device = static_cast<int>(requested_device);
        last_device = first_device + 1;
      }

      for (int device_index = first_device; device_index < last_device; ++device_index)
      {
        EGLDisplay display = get_device_display(EGL_PLATFORM_DEVICE_EXT, devices[device_index], nullptr);
        if (display != EGL_NO_DISPLAY && eglInitialize(display, &major, &minor))
        {
          egl_display_ = display;
          RCLCPP_INFO(node_->get_logger(), "EGL: Selected device %d of %d", device_index, device_count);
          break;
        }
      }
    }
  ...
  // Mesa does not necessarily expose EGL devices, but supports a surfaceless
  // software-rendering display. Keep it as a portable fallback.
  ...
  const EGLint initialize_error = eglGetError();
  if (initialize_error != EGL_SUCCESS)
  {
    RCLCPP_ERROR(node_->get_logger(), "EGL: Failed to initialize (error: 0x%x)", initialize_error);
  ...
  // MuJoCo versions before 3.6 require a default framebuffer while creating
  // their offscreen framebuffer. A tiny pbuffer supplies it without a window.
